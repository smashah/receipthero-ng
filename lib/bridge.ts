...
import { RetryQueue, RETRY_QUEUE_PATH } from "./retry-queue";
...
export async function processPaperlessDocument(
  ...
  const attemptNum = retryQueue ? retryQueue.getAttempts(documentId) + 1 : 1;
  const maxRetries = 3;
  ...
  try {
    ...
    if (retryQueue) {
      ...
      if (retryQueue.shouldGiveUp(documentId)) {
        ...
        retryQueue.add(documentId, errorMessage);
...
export async function runAutomation() {
  ...
  const retryQueue = new RetryQueue(RETRY_QUEUE_PATH, config.processing.maxRetries);
  retryQueue.logStats();
  ...
  for (const doc of unprocessed) {
    await processPaperlessDocument(client, doc.id, retryQueue, config.processing.failedTag);
  }

  // 2. Process documents from retry queue that are ready
  const readyForRetry = retryQueue.getReadyForRetry();
  if (readyForRetry.length > 0) {
    console.log(`Processing ${readyForRetry.length} documents from retry queue`);
    
    for (const item of readyForRetry) {
      await processPaperlessDocument(client, item.documentId, retryQueue, config.processing.failedTag);
    }
  }
}
